...
public class HttpTaskServer {
    ...
    Создайте класс HttpTaskServer — он будет слушать порт $8080$ и принимать запросы.
    Это будет основной класс вашего приложения.
    В нём должен находиться метод main, который будет запускаться для начала работы с программой.

    Чтобы приложение могло принимать и обрабатывать запросы пользователя, используйте класс HttpServer.
    ...
    Добавьте в проект библиотеку Gson для работы с JSON.
    ...
    Добавьте в сервер работу с менеджером задач.
    ...
    В обработчиках реализуйте обработку запросов пользователя, а также обращение к соответствующим
    методам менеджера задач согласно спецификации.
